# @gestisac/hq-web — Esqueleto para separação futura
...
```bash
pnpm run dev:hq     # http://127.0.0.1:5174
pnpm run build:hq
pnpm run typecheck:hq
```

## O que está aqui

- `src/app.tsx` — Componente principal com `PortalFrame` que demonstra o dashboard HQ
